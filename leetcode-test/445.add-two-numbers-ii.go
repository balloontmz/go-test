...
func addTwoNumbers(l1 *ListNode, l2 *ListNode) *ListNode {
	...
	// Your runtime beats 15.38 % of golang submissions
	// Your memory usage beats 100 % of golang submissions (5.7 MB)
	var l1Stack = buildStack(l1)
	var l2Stack = buildStack(l2)
	var tail = 0
	var res = &ListNode{}
	var head = res
	for len(l1Stack) != 0 || len(l2Stack) != 0 || tail != 0 {
		var x, y int
		if len(l1Stack) != 0 {
...
//采用链表反转以及删除链表的方法解两个链表的和--此处耗时较多，调试不够熟练。 -- 题目不允许修改链表，次解法不符合要求！！！
// func addTwoNumbers(l1 *ListNode, l2 *ListNode) *ListNode {
